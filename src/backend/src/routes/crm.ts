...
router.post("/clients", async (req, res) => { try { const { name, email, phone, company } = req.body; if (!name) return res.status(400).json({ error: "name is required" }); const result = await clientModel.create({ name, email, phone, company }); res.status(201).json({ ok: true, id: result.insertId }); } catch (err: any) { res.status(500).json({ error: err.message }); } });
